...
namespace DB_course.Tests
{
    public class WarehouseAdminModelTests
    {
        ...
        public void RemovePlace_InvalidId_ThrowsException()
        {
            ...
            // Act + Assert
            Assert.Throws<NullReferenceException>(() => model.RemovePlace(invalidId));
        ...
        public void RemovePlace_ValidId_DeletesPlaceFromUnitOfWork()
        {
            ...
            var mockPersonRepository = new Mock<IRepository<Place>>();
            unitOfWorkMock.Setup(u => u.PlaceRepository).Returns(mockPersonRepository.Object);
            var mockPORepository = new Mock<IRepository<PlaceofObject>>();
            unitOfWorkMock.Setup(u => u.PlaceofObjectRepository).Returns(mockPORepository.Object);
            var model = new WarehouseAdminModel(unitOfWorkMock.Object);
            // Act
        ...
        public void UpdatePlace_PlaceDoesNotExist_ThrowsException()
        {
            ...
            // Act + Assert
            Assert.Throws<NoSuchObjectException>(() => model.UpdatePlace(nonExistentId, newPlace));
            unitOfWorkMock.Verify(x => x.PlaceRepository.Update(It.IsAny<Place>()), Times.Never);
            unitOfWorkMock.Verify(x => x.PlaceRepository.Save(), Times.Never);
        }
        ...
        public void AddProduct_WithValidInput_ShouldCreateNewProductAndInventory()
        {
            ...
            var Place = PlaceMother.CreateValidPlace1();
            var Place1 = PlaceMother.CreateValidPlace2();
            var Place2 = PlaceMother.CreateValidPlace3();
            ...
            _unitOfWorkMock.Setup(x => x.PlaceRepository.Get("1")).Returns(new List<Place> {Place});
            _unitOfWorkMock.Setup(x => x.PlaceRepository.Get("2")).Returns(new List<Place> { Place1 });
            _unitOfWorkMock.Setup(x => x.PlaceRepository.Get("3")).Returns(new List<Place> { Place2 });
        ...
        public void RemoveProduct_ProductFoundAndHasValueGreaterThanOne_DecreasesValue()
        {
            ...
            var mockUsefulRepository1 = new Mock<IRepository<Useful>>();
            _unitOfWorkMock.Setup(u => u.UsefulRepository).Returns(mockUsefulRepository1.Object);
